...
其中 $U$ 的列向量是 $\Sigma$ 的特征向量，$S$ 的对角元素表示对应方向上的方差大小。PCA 要找的是数据方差最大的方向，因此按 $S$ 中数值从大到小排列时，$U$ 的前几列就是最重要的主成分方向。

例如后文 NumPy 示例中的二维数据经过标准化后，协方差矩阵为：
...
第一列 $\begin{bmatrix}0.7071 & 0.7071\end{bmatrix}^T$ 对应较大的奇异值 $1.9259$，表示数据主要沿 $x_1$ 和 $x_2$ 同时增大的方向变化；第二列对应较小的奇异值 $0.0741$，表示该方向保留的信息很少。特征向量整体乘以 $-1$ 仍表示同一条方向，因此不同软件输出的符号可以不同。
